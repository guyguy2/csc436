...
const Product = class Product {
    // 'Apple,1.00,In Stock'
    constructor (productDesc) {
        const prodArray = productDesc.split(","); 
        this.name = prodArray[0];   
        this.price = Number(prodArray[1]);
    ...
    */
    static halfOff = (products) => {
        ...
        const half = a => a * 0.5;
        const result = products.map(p => ({...p, price:half(p.price)}));
        // const result = products.map(p => Object.assign(p, {price: half(p.price)}));

        
        // let guy1 = products.map(p => p.price * 0.5 );
        
        // let guy2 = products.map(({price})=> { 
        //     price = price * 0.5;
        //     return price; 
        // });
        
        // let guy = products.map(function(element){
    ...
    */
    static printProducts = (products) => {
        products.forEach(p => console.log(`Product: ${p.name}, Cost: ${new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(p.price)}, Availability: ${p.availability == 'In Stock' ? 'Yes' : 'No'}`));
    }
    //console.log(new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(number));
    
};

//console.log(new (Product('Apple,1.00,In Stock')));
